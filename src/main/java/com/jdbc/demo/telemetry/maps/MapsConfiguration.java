...
public class MapsConfiguration {
    ...
    private static String getPropertiesKey(String keyName) throws  NoSuchElementException{
        if(!PROPERTIES.containsKey(keyName))
            throw new NoSuchElementException(String.format("Required key %s not" +
                            "found in Google Maps API configuration file(location: %s).", keyName,
    ...
    private static Properties readProperties() throws FileNotFoundException{
        ...
        try {
            ...
            if (inputStream != null) {
                ...
                throw new FileNotFoundException(String.format("Unable to find %s file in classpath.", PROPERTIES_FILE_NAME));
            }
        } catch (IOException e) {
