...
public class Vehicle {
	...
	@JoinTable(name = "FreightTransportVehicles", joinColumns = {
			@JoinColumn(name = "id_Vehicle") }, inverseJoinColumns = { @JoinColumn(name = "id_FreightTransport") })
	...
	public boolean equals(Object o) {
		...
		if (id != vehicle.id)
			return false;
		if (horsepower != vehicle.horsepower)
			return false;
		...
		if (!model.equals(vehicle.model))
			return false;
		if (!brand.equals(vehicle.brand))
			return false;
		if (!VIN.equals(vehicle.VIN))
			return false;
		if (available != null ? !available.equals(vehicle.available) : vehicle.available != null)
			return false;
	...
	public String toString() {
		return "Vehicle{" + "id=" + id + ", horsepower=" + horsepower + ", engine=" + engine + ", mileage=" + mileage
				+ ", model='" + model + '\'' + ", brand='" + brand + '\'' + ", VIN='" + VIN + '\'' + ", productionDate="
				+ productionDate + ", available=" + available + '}';
	}
	...
	public List<ScheduleEvent> getSchedule(Date start, Date end) {
		...
		if (start.compareTo(end) > 0) {
			throw new IllegalArgumentException(String.format(
					"Attempted to get vehicle's schedule with malformed" + " constraint. Start date: %s, End date: %s",
					start, end));
		}
