	...
	<nav class="navbar navbar-inverse">
		<div class="container-fluid">
			<div class="navbar-header">
				<button model="button" class="navbar-toggle collapsed"
			...
			<c:choose>
				<c:when test="${waypointsString != null}">
					<iframe width="600" height="450" frameborder="0" style="border: 0"
						src="https://www.google.com/maps/embed/v1/directions?origin=${route.originLocation}&destination=${route.destinationLocation}&key=${api_key}&mode=driving&waypoints=${waypointsString}"
						allowfullscreen></iframe>
				</c:when>
				<c:otherwise>
					<iframe width="600" height="450" frameborder="0" style="border: 0"
						src="https://www.google.com/maps/embed/v1/directions?origin=${transport.loadAddress.town}&destination=${transport.unloadAddress.town}&key=${api_key}&mode=driving"
						allowfullscreen></iframe>
				</c:otherwise>
			</c:choose>
			<div class="row">
			...
			<c:choose>
				<c:when test="${transport.finished == true}">
					<span class="label label-success"> <span
						class="glyphicon glyphicon-ok" aria-hidden="true"></span>Zakończony
					</span>
				</c:when>
				<c:otherwise>
					<span class="label label-default"> <span
						class="glyphicon glyphicon-road" aria-hidden="true"></span>W
						trakcie
					</span>
				</c:otherwise>
			</c:choose>
			...
			<div class="row list-group">
				...
				<c:forEach var="driver" items="${drivers}">
					<a href="/drivers/${driver.id}" class="list-group-item">
						<h4 class="list-group-item-text">${driver.firstName}
							${driver.lastName} (${driver.PESEL})</h4>
					</a>
				</c:forEach>
			</div>
			...
			<div class="row list-group">
				...
				<c:forEach var="vehicle" items="${vehicles}">
					<a href="/vehicles/${vehicle.id}" class="list-group-item">
						<h4 class="list-group-item-text">${vehicle.brand}
							${vehicle.model} (${vehicle.VIN})</h4>
					</a>
				</c:forEach>
			</div>
		</div>
